...
export default function MessageSearch({ conversationId, onMessageFound }: MessageSearchProps) {
  ...
  const { data: searchResultsResponse, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      return conversationService.searchMessages(conversationId, user.id, query.trim());
        ...
        if (currentUser?.id) {
          const { settings, error } = await settingsService.getUserSettings(currentUser.id);
          if (settings && !error) {
            setTheme(settings.theme as Theme || "light");
            setAccent(settings.accentColor as AccentColor || "teal");
            setBackground(settings.backgroundColor as BackgroundColor || "lightGrey");
            setChatBackground(settings.chatBackground as ChatBackground || "solid");
            setChatBackgroundColor(settings.chatBackgroundColor as BackgroundColor || "lightGrey");
          }
  const executeSearch = () => {
  ...
  // Clear all filters and search query
      ...
      if (currentUser?.id) {
        await settingsService.updateUserSettings(currentUser.id, {
          theme,
          accentColor: accent,
